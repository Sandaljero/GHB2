...
Функция ТаблицаВсехТаблицБДЛкс() Экспорт 
	...
	СостояниеРасчета = ирКэш.СостояниеПодготовкиКэшаЛкс();
	...
	Если ФоновоеЗадание <> Неопределено Тогда 
		...
		Если ФоновоеЗадание.Состояние = СостояниеФоновогоЗадания.Завершено Тогда 
			СостояниеРасчета.ФормаРезультата.ОбновитьВременноеХранилище(СостояниеРасчета.АдресРезультата);
			ТаблицаВсехТаблицБД = ПолучитьИзВременногоХранилища(СостояниеРасчета.АдресРезультата);
			УдалитьИзВременногоХранилища(СостояниеРасчета.АдресРезультата);
			Если ТаблицаВсехТаблицБД = Неопределено Тогда
				Сообщить("Не удалось получить результат фонового расчета таблицы всех таблиц БД. Выполняем расчет в основном потоке.");
			КонецЕсли; 
		КонецЕсли; 
	КонецЕсли; 
	Если ТаблицаВсехТаблицБД = Неопределено Тогда 
...
Функция ПолучитьСеансТонкогоКлиентаЛкс() Экспорт
	...
	Исключение
		ирОбщий.СообщитьЛкс("Не удалось подключить тонкий клиент через COM", СтатусСообщения.Внимание);
		...
		Для Каждого СтрокаМетаТипа Из ТаблицаТиповМетаОбъектов Цикл
			...
			Для Каждого МетаОбъект Из Метаданные[СтрокаМетаТипа.Множественное] Цикл
				Если МетаОбъект.РасширениеКонфигурации() <> Неопределено Тогда
					ТипыРасширений.Добавить(Тип(ирОбщий.ИмяТипаИзПолногоИмениМДЛкс(МетаОбъект)));
				КонецЕсли;
			КонецЦикла;
		КонецЦикла;
	КонецЕсли;
...
Функция СостояниеПодготовкиКэшаЛкс() Экспорт 
	Если Ложь
	...
	ФормаРезультата = ирОбщий.ФормаРезультатаФоновогоЗаданияЛкс();
...
Функция ВременноеОкноСообщенийЛкс() Экспорт 
	
	//Форма = ПолучитьФорму("Обработка.ирПлатформа.Форма.ВременноеОкноСообщений");
	мПлатформа = ирКэш.Получить();
	#Если Сервер И Не Сервер Тогда
		мПлатформа = Обработки.ирПлатформа.Создать();
	#КонецЕсли
	Форма = мПлатформа.ПолучитьФорму("ВременноеОкноСообщений");
	Возврат Форма;
