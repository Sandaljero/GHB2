...
Функция ПолучитьПрокси(ВызыватьИсключение = Истина, ИмяПользователя = Неопределено, Пароль = Неопределено,
	...
	Попытка
		Прокси = Новый WSПрокси(Определения,
			//++Voitekho.k  04.06.2020 у нас используется другой пакет XDTO под другим именем
			//"http://www.1c.ru/dm",
			"http://www.1c.ru/dme",
			//--Voitekho.k 
			"DMService",
...
Функция СоздатьОбъект(Прокси, ТипОбъекта) Экспорт
	...
	НовыйОбъект = Прокси.ФабрикаXDTO.Создать(Прокси.ФабрикаXDTO.Тип("http://www.1c.ru/dme", ТипОбъекта));
	//--Voitekho.k 
...
Функция ПроверитьТип(Прокси, ОбъектXDTO, Тип) Экспорт
	...
	Если ОбъектXDTO.Тип() = Прокси.ФабрикаXDTO.Тип("http://www.1c.ru/dme", Тип) Тогда
	//--Voitekho.k 	
		Возврат Истина;
	Иначе
		Возврат Ложь;
...
Функция ЭтоОбъектноеСвойство(Прокси, ОбъектXDTO, ИмяСвойстваXDTO) Экспорт
	...
	ТипРодитель = Прокси.ФабрикаXDTO.Тип("http://www.1c.ru/dme", "DMObject");
	//--Voitekho.k 
...
Функция ЭтоОбъектноеЗначение(Прокси, Значение) Экспорт
	...
	ОбъектныйТип = Прокси.ФабрикаXDTO.Тип("http://www.1c.ru/dme", "DMObject");
	//--Voitekho.k 
...
Процедура ЗаполнитьСвойствоXDTOизСтруктурыРеквизитов(Прокси, ОбъектXDTO, ИмяСвойства, СтруктураРеквизитов, ПутьДанных) Экспорт
	...
	Если ТипЗнч(ПутьДанных) = Тип("Структура") Тогда
		...
		//++Voitekho.k  15.06.2020 #13897
		// из-за различия в пакетах DM-DME нет некоторых свойств, которые ожидаются как обязательные
		// Переписал код так, чтобы при отсутствии свойства в объекте код не падал.
		...
		Если Свойство = Неопределено Тогда
			Возврат;
		КонецЕсли;
		СвойствоКонтейнер = СоздатьОбъект(Прокси, Свойство.Тип.Имя);
		//--Voitekho.k 
