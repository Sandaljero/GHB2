...
Функция ОткрытьДляОтладки(Знач СхемаИлиМакетКомпоновки = Неопределено, Знач Настройки = Неопределено, Знач ВнешниеНаборыДанных = Неопределено,
	...
	Если Настройки <> Неопределено Тогда
		ПодготовитьЗначенияОтбора(Настройки.Отбор);
...
Процедура ПодготовитьЗначенияОтбора(Знач ГруппаОтбора)
	...
	Для Каждого ЭлементОтбора Из ГруппаОтбора.Элементы Цикл
		...
		Если МассивЗначенияОтбора <> Неопределено Тогда
			...
			Сообщить("Значение отбора элемента """ + ЭлементОтбора.ЛевоеЗначение + """ было преобразовано из массива в список значений", СтатусСообщения.Внимание);
		КонецЕсли; 
	КонецЦикла;
