...
//
Процедура ПрочитатьНастройкиЖурнала(Индекс, выхМестоположение, выхВремяХранения, выхПредставлениеСобытий = Неопределено, выхПредставлениеСвойств = Неопределено) Экспорт
...
Процедура ПрочитатьНастройкиЖурналов(ТабличноеПолеЖурналы) Экспорт
	...
	Для Инд = 0 По УзлыЖурналов.Количество() - 1 Цикл
		...
		ПрочитатьНастройкиЖурнала(Инд, Журнал.Местоположение, Журнал.ВремяХранения, Журнал.События, Журнал.Свойства);
...
Процедура ПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки) Экспорт
	...
	Если ЗначениеЗаполнено(ДанныеСтроки.Свойство) Тогда
		...
		Если СтруктураТиповЗначенийСвойств.Свойство(СтрЗаменить(ДанныеСтроки.Свойство, ":", "_"), ТипЗначенияСвойства) Тогда
			ОформлениеСтроки.Ячейки.Значение.УстановитьКартинку(ирОбщий.ПолучитьПиктограммуТипаЛкс(ТипЗначенияСвойства.Типы()[0]));
		КонецЕсли; 
	КонецЕсли; 
