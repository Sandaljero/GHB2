...
Функция ВыбратьСтрокуОписанияИзМассиваСтруктурТипов(СтруктураЦикла, ВключатьПутьКОписаниюТипаЗначения = Ложь, ФормаВладелец = Неопределено, Слово = "",
	НомерПараметраМетода = 0, БезусловнаяАктивизацияРезультатов = Истина, выхФормаВыбора = Неопределено) Экспорт
	...
	Если СчетчикСтрокВторогоУровня > 0 Или выхФормаВыбора.Открыта() Тогда
		...
		выхФормаВыбора.ВладелецФормы = ФормаВладелец;
...
Функция ПолучитьВнутреннююТаблицуМетаданныхСлов(Знач РодительскаяСтруктураТипа, Знач Слово = Неопределено, Знач ТипСлова = Неопределено, Знач ВиртуальнаяТаблица = Неопределено, Знач ЯзыкПрограммы = 0,
	...
	Если Ложь
		...
		Или ЯзыкПрограммы = 2
	Тогда
		ДобавитьВТаблицуСловЭлементыМодуляМетаданных(СтрокаОбщегоТипа, Слово, ТипСлова, ВнутренняяТаблицаСлов);
	КонецЕсли; 
	Если Истина
...
Функция ДобавитьВТаблицуСловЭлементыМодуляМетаданных(Знач СтруктураТипа, Знач Слово = Неопределено, Знач ТипСлова = Неопределено, Знач ВнутренняяТаблицаСлов = Неопределено,
	КомпонентаПодсказки = Неопределено) Экспорт 
	...
	Если ТипЗнч(МетаданныеРодителя) = Тип("ОбъектМетаданных") Тогда 
		...
		Если ЗначениеЗаполнено(ирОбщий.ПервыйФрагментЛкс(ИмяОбщегоТипа, "Объект.", Ложь)) Тогда
		...
		Если ТипМодуля <> Неопределено Тогда
			...
			Если МодульМетаданных = Неопределено Тогда
				...
				Если ФайлОписанияМодуляСуществует Тогда
					...
					Если ТипЗнч(МодульМетаданных) <> Тип("Структура") Тогда
	...
	Если МодульМетаданных <> Неопределено Тогда
		...
		Тогда
			...
			Для Каждого МетодМодуля Из ПодходящиеМетоды Цикл
				...
				Тогда
					ПодготовитьТипРезультатаМетода(МетодМодуля, МодульМетаданных, ИскательВозврата, КомпонентаПодсказки); 
				КонецЕсли;
				ТаблицаСтруктурТипа = НоваяТаблицаСтруктурТипа();
				Если МетодМодуля.ТаблицаСтруктурТипов <> Неопределено Тогда
					ТаблицаСтруктурТипа = МетодМодуля.ТаблицаСтруктурТипов.Скопировать();
					#Если Сервер И Не Сервер Тогда
						ТаблицаСтруктурТипа = Новый ТаблицаЗначений;
					#КонецЕсли
				Иначе
					СтруктураТипа = ТаблицаСтруктурТипа.Добавить();
					Если МетодМодуля.Тип = "Функция" Тогда
						СтруктураТипа.ИмяОбщегоТипа = "??";
					КонецЕсли; 
				КонецЕсли; 
				ТаблицаСтруктурТипа.ЗаполнитьЗначения(МетодМодуля, "СтрокаОписания");
				ДобавитьВТаблицуСлов(ВнутренняяТаблицаСлов, МетодМодуля.Имя, "Метод", ТаблицаСтруктурТипа);
			КонецЦикла;
			Если Индикатор <> Неопределено Тогда
...
Функция ПодготовитьТипРезультатаМетода(СтрокаМетода, МодульМетаданных, кэшИскательВозврата = Неопределено, кэшКомпонентаПодсказки = Неопределено) Экспорт 
	...
	Если СтрокаМетода.Тип = "Функция" И СтрокаМетода.ТаблицаСтруктурТипов = Неопределено Тогда
		...
		КонецЕсли; 
		СтрокаМетода.Тело = ""; // Освобождаем память
	КонецЕсли;
...
Функция ПолучитьВнутреннююТаблицуПредопределенныхСлов(РодительскаяСтруктураТипа,
	...
	Для Каждого СтрокаОбщегоТипа Из МассивОбщихТипов Цикл
		...
		Если НайденныеСтроки = Неопределено Тогда 
			...
			КонецЕсли;
			Если ИмяОбщегоТипа = "Локальный контекст" Тогда
...
//
Процедура _Sleep(КоличествоСекунд) Экспорт
	...
	Иначе
		...
		Иначе
			...
			Если КоличествоПереводов > 1 Тогда
				ирОбщий.СообщитьЛкс("Неоднозначный перевод на русский идентификатора """ + Локальный + """", СтатусСообщения.Внимание);
			КонецЕсли; 
		КонецЕсли; 
		Возврат Русский;
	КонецЕсли; 
