...
Процедура Уничтожить() Экспорт
	...
	КонецЦикла;
	ЭтотОбъект.ФормаПодсказки = Неопределено;
	ОчиститьТаблицуСловЛокальногоКонтекста();
...
Функция Нажатие(Кнопка) Экспорт
	...
	ИначеЕсли Команда = "ВводСкобки" Тогда
...
//
Функция ОткрытьПоискВСинтаксПомощнике(ПутьКСлову, СтрокаОписания = Неопределено, ВключатьПутьКОписаниюТипаЗначения = Ложь, выхФормаВыбора = Неопределено, РазрешитьАнализИмениТипа = Истина) Экспорт

	МассивЗащитыОтРекурсии.Очистить();
	ТаблицаСтруктурТиповКонтекста = ОпределитьТипЗначенияКонтекста(ПутьКСлову, " " + мТекстДляПоискаОпределения, мПредшествующийТекст, РазрешитьАнализИмениТипа);
	...
	Иначе
		...
		#Если Сервер И Не Сервер Тогда
			мПлатформа = Обработки.ирПлатформа.Создать();
		#КонецЕсли
		мПлатформа.ВыбратьСтрокуОписанияИзМассиваСтруктурТипов(СтруктураЦикла, ВключатьПутьКОписаниюТипаЗначения, ФормаВладелец, Слово, мНомерПараметра,, выхФормаВыбора);
	КонецЕсли; 
...
Процедура ОткрытьКонтекстнуюСправку(ПутьКСлову = "") Экспорт
	...
	КонецЕсли;
	ОткрытьПоискВСинтаксПомощнике(ПутьКСлову,,,, РазобратьТекущийКонтекст);
...
Процедура ОткрытьСправкуПоПараметру(ПутьКСлову = "", ОткрыватьСинтаксПомощник = Истина) Экспорт
	...
	Если ОткрыватьСинтаксПомощник Тогда
		...
		ОткрытьПоискВСинтаксПомощнике(ПутьКСлову,,, ФормаСинтаксПомощникаПосле);
	КонецЕсли;
	ФормаПодсказкаПоПараметрам = ПолучитьФорму("ВызовМетода");
...
//   Строка.
//
Функция ЗалитьКомментарииИСтроковыеЛитералы(Знач Текст, СимволЗамена = " ") Экспорт 
	...
	ИначеЕсли ЯзыкПрограммы = 0 Тогда
		...
		RegExp.Pattern = "(""" + шИмя+ "(?:\.[А-Яа-я_A-Za-z\d]+)*"")" // имена ProgId от COM объектов бывают "Forms.TextBox.1"
			...
			//+ шОднострочнаяСтрокаПрограммыСКавычки + "|" + шОднострочнаяСтрокаПрограммыСЧерты + "|(.|\n|\r)";
			//+ шОднострочнаяСтрокаПрограммыСКавычки + "|" + шОднострочнаяСтрокаПрограммыСЧерты + "|([\s\S])"; //Закомментировано по http://www.hostedredmine.com/issues/885452
			+ "|([\s\S])";
		//ШаблонЗамены = "$1$2$3$4$5"; //Закомментировано по http://www.hostedredmine.com/issues/885452
		ШаблонЗамены = "$1$2$3$4";
	КонецЕсли;
...
Процедура РазобратьТекущийКонтекст(ЛиСправаОтРавенства = Ложь, выхЕстьТочкаСправа = Ложь)
	...
	Если Результат.Count > 0 Тогда
		...
		RegExp.Global = Истина;
		//RegExp.Pattern = шСтрокаПрограммы + "|(" + шНачалоСтрокиПрограммы + ")$";
		RegExp.Pattern = шСтрокаПрограммы;
		Результат = RegExp.Execute(ТекущееНачалоСтроки);
		Если Результат.Count > 0 Тогда
			ПоследнееВхождение = Результат.Item(Результат.Count - 1);
			мЭтоСтроковыйЛитерал = Истина
				И Прав(ПоследнееВхождение.Value, 1) <> """"
				И ПоследнееВхождение.FirstIndex + ПоследнееВхождение.Length = СтрДлина(ТекущееНачалоСтроки);
		КонецЕсли; 
		RegExp.Global = Ложь;
	КонецЕсли; 
	...
	Иначе 
		RegExp.Pattern = шВызовМетодаПрограммы + "$";
	КонецЕсли;
	Результат = RegExp.Execute(мПредшествующийТекст);
	Если Результат.Count > 0 Тогда
		мВызовМетода = Результат.Item(0).SubMatches(0) + "(";
	...
	Иначе
		СтрокаШаблона = шИмя;
	КонецЕсли;
	СтрокаШаблона = "(((" + шИмя + шСкобки + "?" + "((\." + шИмя + шСкобки + "?)|" + шИндекс + ")*))\.)?(" + СтрокаШаблона + ")?$";
...
Процедура ПерейтиКОпределению()
	...
	Иначе
		...
		ТаблицаСтруктурТипов = ОпределитьТипЗначенияКонтекста(мКонтекст,, мПредшествующийТекст, Истина);
...
//
Функция ОпределитьТипЗначенияКонтекста(Знач Контекст = "", Знач ТекстДляПоискаОпределения = "", Знач ПредшествующийТекст = "", Знач РазрешитьАнализИмениТипа = Ложь) Экспорт
					...
					Если Не ирОбщий.ЛиПустаяПодгруппаRegExpЛкс(ПоследнееВхождение.SubMatches(1)) Тогда
						...
						Если НРег(ПоследнееВхождение.SubMatches(3)) = Нрег("Новый") Тогда
							...
							Тогда
								...
								КонецЦикла;
							ИначеЕсли Ложь
								Или СтруктураТипа.ИмяОбщегоТипа = "Запрос"
								Или СтруктураТипа.ИмяОбщегоТипа = "ПостроительЗапроса"
								Или СтруктураТипа.ИмяОбщегоТипа = "ПостроительОтчета"
							Тогда
								RegExp.Global = Ложь;
								ТекстДляЗаполненияМетаданных = Сред(ТекстДляПоискаОпределения, ПоследнееВхождение.FirstIndex + ПоследнееВхождение.Length + 1);
								СтруктураТипа.Метаданные = Новый ТаблицаЗначений;
								...
								Если ЗначениеЗаполнено(ТекстЗапроса) Тогда
									...
									Если ПостроительЗапроса <> Неопределено Тогда
										ирОбщий.УстановитьМетаданныеКоллекцииЛкс(ПостроительЗапроса.ДоступныеПоля, СтруктураТипа.Метаданные);
									КонецЕсли; 
								КонецЕсли; 
							КонецЕсли; 
						...
						ИначеЕсли Не ирОбщий.ЛиПустаяПодгруппаRegExpЛкс(ПоследнееВхождение.SubMatches(6)) Тогда 
					        ТаблицаСтруктурТипов = ОпределитьТипЗначенияКонтекста(ПрисвоенныйКонтекст, Лев(ТекстДляПоискаОпределения,
								ПоследнееВхождение.FirstIndex - 1) , Лев(ТекстДляПоискаОпределения, ПоследнееВхождение.FirstIndex - 1), РазрешитьАнализИмениТипа);
							Для Каждого СтруктураТипа Из ТаблицаСтруктурТипов Цикл
...
Процедура ВызватьКонтекстнуюПодсказку()
	...
	Пока ТаблицаСтруктурТиповКонтекста.Количество() > 0 Цикл
		...
		Если ВыбраннаяСтруктураТипа.ТипЯзыка <> "ИмяТипа" И мЭтоСтроковыйЛитерал Тогда 
			...
			Если ВыбраннаяСтруктураТипа.СтрокаОписания <> Неопределено Тогда 
				Если ВыбраннаяСтруктураТипа.СтрокаОписания.Владелец().Колонки.Найти("ЛиЭкспорт") = Неопределено Тогда
				...
				Иначе
					СтрокиПараметров = ВыбраннаяСтруктураТипа.СтрокаОписания.Параметры;
				КонецЕсли; 
				Если СтрокиПараметров.Количество() >= мНомерПараметра Тогда
					СтрокаПараметра = СтрокиПараметров[мНомерПараметра - 1];
				...
				ИначеЕсли ТипЗнч(ВыбраннаяСтруктураТипа.Метаданные) = Тип("ОбъектМетаданных") Тогда 
					ИмяТаблицыБД = ирОбщий.ИмяТаблицыИзМетаданныхЛкс(ВыбраннаяСтруктураТипа.Метаданные.ПолноеИмя());
			...
			ИначеЕсли ВыбраннаяСтруктураТипа.ТипЯзыка = "ПространствоБлокировки" Тогда
				КоллекцияКолонок = Новый Массив;
				Если ЗначениеЗаполнено(мРодительскийКонтекст) Тогда
					ТаблицаТиповМетаОбъектов = мПлатформа.ПолучитьТаблицуТиповМетаОбъектов(Ложь);
					...
					Если Найти(мРодительскийКонтекст, ".") > 0 Тогда
						...
						КонецЕсли; 
					Иначе
						СтрокаТипаМетаОбъектов = ТаблицаТиповМетаОбъектов.Найти(КорневойТип, "Единственное");
						Если СтрокаТипаМетаОбъектов <> Неопределено Тогда
							Для Каждого МетаОбъект Из Метаданные[СтрокаТипаМетаОбъектов.Множественное] Цикл
								КоллекцияКолонок.Добавить(МетаОбъект);
							КонецЦикла;
						КонецЕсли; 
					КонецЕсли; 
				Иначе
					Для Каждого СтрокаТипаМетаОбъекта Из мПлатформа.ПолучитьТаблицуТиповМетаОбъектов(Ложь, Ложь, Ложь) Цикл
				...
				Иначе
					Если ОбъектМД <> Неопределено Тогда
						...
						Если Найти(ПространствоБлокировки, "." + "НаборЗаписей") > 0 Тогда
							СтруктураКлюча = ирОбщий.СтруктураКлючаТаблицыБДЛкс(ирОбщий.ИмяТаблицыИзМетаданныхЛкс(ОбъектМД), Ложь);
						Иначе
		...
		Если ЗначениеЗаполнено(СтруктураТипаКонтекста.ТипЯзыка) Тогда
			ФормаПодсказки.Контекст = СтруктураТипаКонтекста.ТипЯзыка;
		Иначе
		...
		Если СтрокаРезультата.ТипСлова = "Метод" Тогда
			...
			Если ПараметрЗакрытияПодсказки = Истина Тогда
				...
				Если ТаблицаМетодовМодуля.Количество() > 0 Тогда
					Если ТаблицаМетодовМодуля[0].ТаблицаСтруктурТипов[0].СтрокаОписания.Параметры <> Неопределено Тогда
...
//
Функция ПолучитьПредставлениеМассиваСтруктурТипов(ТаблицаСтруктурТиповКонтекста) Экспорт 
